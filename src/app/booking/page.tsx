...
export default function BookingPage() {
  ...
  const loadEventData = useCallback(async () => {
    try {
      ...
      setDebugInfo(prev => prev + `Ищем событие по slug: ${eventSlug}\n`)
      ...
      if (eventError) {
        ...
        throw new Error(`Событие "${eventSlug}" не найдено или неактивно`)
      }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mt-6 bg-green-50 p-4 rounded-md">
          <h3 className="font-semibold mb-2">🎉 Диагностическая информация (Query Params):</h3>
          <div className="text-sm space-y-1">
            <p><strong>URL:</strong> {typeof window !== 'undefined' ? window.location.href : 'SSR'}</p>
            <p><strong>Event Slug:</strong> {eventSlug || 'не указан'}</p>
