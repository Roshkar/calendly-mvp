...
export default function BookingPage({ params }: { params: { username: string, eventType: string } }) {
  ...
  const loadEventData = async () => {
    try {
      ...
      // Ищем профиль пользователя по username
      ...
      setDebugInfo(prev => prev + `✅ Пользователь найден: ${profile.first_name} ${profile.last_name}\n`)

      // Ищем событие по slug
      setDebugInfo(prev => prev + `Ищем событие: ${params.eventType}\n`)
      
      const { data: event, error: eventError } = await supabase
        .from('event_types')
        .select('*')
        .eq('user_id', profile.id)
        .eq('slug', params.eventType)
        .eq('is_active', true)
        .single()

      if (eventError) {
        setDebugInfo(prev => prev + `❌ Событие не найдено: ${eventError.message}\n`)
        throw new Error(`Событие "${params.eventType}" не найдено или неактивно`)
      }
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      setDebugInfo(prev => prev + `Данные бронирования: ${JSON.stringify(bookingData, null, 2)}\n`)
      ...
      if (bookingError) {
        setDebugInfo(prev => prev + `❌ Ошибка бронирования: ${bookingError.message}\n`)
        throw new Error('Ошибка при создании бронирования: ' + bookingError.message)
      }

      setDebugInfo(prev => prev + `✅ Бронирование создано: ${JSON.stringify(data, null, 2)}\n`)
      setSuccess(true)
          ...
          <div className="bg-white rounded-lg border p-6">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-semibold">
                  {userProfile?.first_name} {userProfile?.last_name} (@{params.username})
                </h2>
            ...
            <div className="mt-6 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-700">
                {eventData?.location_type === 'online' 
                  ? 'Ссылка на встречу будет отправлена на ваш email после бронирования.'
                  : eventData?.location_type === 'phone'
                  ? 'Мы свяжемся с вами по указанному номеру телефона.'
                  : 'Адрес встречи будет отправлен на ваш email.'}
              </p>
            </div>
          </div>
